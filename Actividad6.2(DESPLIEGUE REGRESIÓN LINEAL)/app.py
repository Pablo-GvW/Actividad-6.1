...
def load_data_2():
    ...
    Lista2 = ["Administrador", "botón correcto", "mini juego", "color presionado", "dificultad", "Juego", 
             "auto push", "tiempo de interacción", "tiempo de lección", "tiempo de sesión"]
...
if View == "Extracción de características 🔠":
    ...
    options=Lista)
    st.title(f"Análisis de {Variable} por usuario")
    ...
    for usuario in usuarios_validos:
        ...
        fig = px.bar(
            ...
            x="categorias",
            y="frecuencia",
            title=f"Frecuencia de {Variable} - Usuario {usuario}",
    ...
    with tab_usuario:
        ...
        with st.container():
            ...
            if not df_usuario2.empty:
                if st.checkbox(f"Mostrar mapa de calor{usuario_seleccionado2} ☄️", key="general_heatmap3"):
...
if View == "Regresión Lineal Multiple 🔧":
    ...
    with tab_general:
        ...
        if st.checkbox(f"Mostrar mapa de calor ☄️", key="general_heatmap4"):
            Corr_Factors2_abs = abs(df2.corr())
            plt.figure(figsize=(10,8))
            sns.heatmap(Corr_Factors2_abs, cmap='Oranges', annot=True, fmt='.2f')
            st.pyplot(plt)
